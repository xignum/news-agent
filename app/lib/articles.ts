...
// Instead of returning just an array of articles, we now return this
// small "report" object: the articles, plus how many feeds failed.
...
export async function getArticles(feedUrls: string[]): Promise<ArticlesResult> {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === "fulfilled") {
      ...
      failedFeedCount = failedFeedCount + 1;
